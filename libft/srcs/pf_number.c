...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/06/19 02:26:18 by smadesi          #+#    #+#              */
...
void pf_putnb(va_list ap, t_printf *p)
{
    ...
    if (p->lm.llong)
        n = (p->lm.llong >> 1) ? ((intmax_t)va_arg(ap, long long)) : ((intmax_t)va_arg(ap, long));
    else if (p->lm.sshort)
        n = (p->lm.sshort >> 1) ? (intmax_t)((char)va_arg(ap, int)) : (intmax_t)((short)va_arg(ap, int));
    else if (p->lm.intmax)
...
void pf_putnb_base(int base, va_list ap, t_printf *p)
{
    ...
    if (p->lm.llong)
        n = (p->lm.llong >> 1) ? ((intmax_t)va_arg(ap, unsigned long long)) : ((intmax_t)va_arg(ap, unsigned long));
    else if (p->lm.sshort)
        n = (p->lm.sshort >> 1) ? (uintmax_t)((unsigned char)va_arg(ap, int)) : (uintmax_t)((unsigned short)va_arg(ap, int));
    else if (p->lm.intmax)
